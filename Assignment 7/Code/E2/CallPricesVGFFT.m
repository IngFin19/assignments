function pricesFFT = CallPricesVGFFT(forward, discount, moneyness, timeToMaturity, sigma, k, eta,numericalMethodParameters)
%function that compute the price of the european call with FFT in the VG framework
%
%INPUT
...
%  _ pricesFFT = prices of european calls with FFT
  ...
  LaplaceExp= @(w) -timeToMaturity/k * log(1+k*w*sigma^2); %VG
  phi= @(x) exp(-1i*x.*LaplaceExp(eta)).*exp(LaplaceExp( (x.^2+1i*(1+eta*2)*x)/2)); 
